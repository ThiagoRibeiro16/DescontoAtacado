...
var listaDescontos = new List<DescontoItem>();
decimal valorSubtotal, valorTotalDescontos, valorTotal;

Console.WriteLine("--- Desconto no Atacado ---\n");

foreach (var grupoItens in listaItens.GroupBy(item => item.Gtin))
{
    var produto = listaProdutos.Single(prod => prod.Gtin == grupoItens.Key);

    int quantidadeTotal = (int)listaItens
        .Where(item => item.Gtin == grupoItens.Key)
        .Sum(item => item.Quantidade);

    if (quantidadeTotal >= produto.UnidadesAtacado)
    {
        listaDescontos.Add(new DescontoItem
        {
            Gtin = grupoItens.Key,
            Desconto = Math.Abs((produto.PrecoAtacado ?? 0) - produto.PrecoVarejo) * quantidadeTotal,
        });
    }
}

Console.WriteLine("Descontos:");
foreach (var descontoItem in listaDescontos)
{
    Console.WriteLine($"{descontoItem.Gtin,13}   {descontoItem.Desconto,12:C2}");
}

valorSubtotal = listaItens.Sum(item => item.Parcial);
valorTotalDescontos = listaDescontos.Sum(desc => desc.Desconto);
valorTotal = valorSubtotal - valorTotalDescontos;

Console.WriteLine();
Console.WriteLine($"(+) Subtotal  = {valorSubtotal,12:C2}");
Console.WriteLine($"(-) Descontos = {valorTotalDescontos,12:C2}");
Console.WriteLine($"(=) Total     = {valorTotal,12:C2}");
